...
;; AS.fe					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}File Encoding" )
...
{ ;;;; program
AS.plist				:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}manage projects.." )
AS.pset					:= "sendinput, ^!s "			 ;;program settings
AS.pconf				:= "sendinput, ^!+s"			 ;;project configuration
AS.pkey					:= Func( "F_RunAction" ).Bind( "^!s", 1200, "{text}Keymap" )
AS.pk					:= AS.pkey
...
AS.fsa					:= "sendinput, ^s"			 ;;save all
AS.fca					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}Close All" ) ;;close all
AS.fe					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}file encoding" )
}
...
{ ;;;; build
AS.bp					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}Build Apks") ;;build & run
AS.bp					:= "sendinput, ^{F9}"				;;build
AS.brun					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}Run Run/Debug") ;;build & run
AS.bre					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}Rebuild Project") ;;rebuild
...
AS.w					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}Tool Windows")				;;window list
...
{ ;;;; tool
AS.tex					:= Func( "F_RunAction" ).Bind( "^+a", 500, "{text}Show in Explorer")	;;explorer
AS.te					:= AS.tex
